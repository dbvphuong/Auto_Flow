...
class FlowImageView(QWidget):
    ...
    def init_ui(self):
        ...
        self.chk_1k = QCheckBox("1K")
        self.chk_2k = QCheckBox("2K")
        self.chk_4k = QCheckBox("4K")
        self.chk_1k.setChecked(True)
        quality_layout.addWidget(self.chk_1k)
        quality_layout.addWidget(self.chk_2k)
        ...
        config_layout.addWidget(QLabel("Số lượng ảnh / prompt:"), 2, 1)
        ...
        self.chk_1k.stateChanged.connect(self.save_config)
        self.chk_2k.stateChanged.connect(self.save_config)
    ...
    def load_sessions(self):
        ...
        db = SessionLocal()
        sessions = db.query(ImageSession).order_by(ImageSession.id.asc()).all()
        
        # Nếu chưa có phiên nào, tạo 6 phiên mặc định
        if not sessions:
            for i in range(1, 7):
                new_sess = ImageSession(name=f"Phiên {i}", ref_dir="", save_dir="", prompts_text="", status="PENDING")
                db.add(new_sess)
            db.commit()
            sessions = db.query(ImageSession).order_by(ImageSession.id.asc()).all()
    ...
    def add_session_row(self):
        db = SessionLocal()
        count = db.query(ImageSession).count()
        new_sess = ImageSession(name=f"Phiên {count + 1}", ref_dir="", save_dir="", prompts_text="", status="PENDING")
        db.add(new_sess)
        db.commit()
    ...
    def delete_selected_sessions(self):
        ...
        for row in rows_to_delete:
            ...
            if item:
                ...
                if sess_id:
                    sess = db.query(ImageSession).filter(ImageSession.id == sess_id).first()
                    if sess:
                        # Xóa các task con liên kết trước
                        db.query(Task).filter(Task.session_id == sess_id, Task.task_type == "image").delete()
                        db.delete(sess)
    ...
    def edit_session_prompts_by_id(self, sess_id):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            for imported_session_id in session_ids_to_sync:
                self.sync_session_tasks(imported_session_id, "image")
            self.load_sessions()
            self.load_tasks()
            return
        db.close()
    ...
    def load_tasks(self):
        ...
        query = db.query(Task).filter(Task.task_type == "image")
    ...
    def add_to_queue(self):
        ...
        for p in prompts:
            ...
            db.add(Task(prompt=p, status="PENDING", task_type="image"))
        db.commit()
        db.close()
        self.load_tasks()
    ...
    def sync_session_tasks(self, sess_id, task_type="image"):
        db = SessionLocal()
        sess = db.query(ImageSession).filter(ImageSession.id == sess_id).first()
        if not sess:
            db.close()
    ...
    def start_batch_sessions(self):
        ...
        for sess_id in selected_sess_ids:
            if self.sync_session_tasks(sess_id, "image"):
                sess = db.query(ImageSession).filter(ImageSession.id == sess_id).first()
                if sess:
                    sess.status = "PENDING"
                    db.commit()
    ...
    def run_next_batch_session(self):
        ...
        db = SessionLocal()
        current_sess = db.query(ImageSession).filter(ImageSession.id == sess_id).first()
        all_tasks = db.query(Task).filter(Task.session_id == sess_id, Task.task_type == "image").order_by(Task.id.asc()).all()
        ...
        for t in tasks:
            task_config = {
                ...
                "quality": [q.text() for q in [self.chk_1k, self.chk_2k, self.chk_4k] if q.isChecked()],
    ...
    def reset_running_tasks_to_pending(self):
        db = SessionLocal()
        try:
            db.query(Task).filter(Task.status == "RUNNING", Task.task_type == "image").update({"status": "PENDING"}, synchronize_session=False)
            db.commit()
        finally:
            db.close()
        for i in range(self.table_tasks.rowCount()):
    ...
    def delete_all_tasks(self):
        ...
        db = SessionLocal()
        self.table_tasks.setRowCount(0)
        db.query(Task).filter(Task.task_type == "image").delete(synchronize_session=False)
        db.commit()
        db.close()
    ...
    def get_config_path(self):
        import os
        src_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        return os.path.join(src_dir, "data", "config_image.json")

    def save_config(self):
        import json
        config = {
            "model": self.combo_model.currentText(),
            "quality_1k": self.chk_1k.isChecked(),
            "quality_2k": self.chk_2k.isChecked(),
    ...
    def load_config(self):
        ...
        try:
            ...
            self.chk_1k.blockSignals(True)
            self.chk_2k.blockSignals(True)
            ...
            self.chk_1k.setChecked(config.get("quality_1k", True))
            self.chk_2k.setChecked(config.get("quality_2k", False))
            ...
            self.chk_1k.blockSignals(False)
            self.chk_2k.blockSignals(False)
    ...
    def load_real_data(self):
        ...
        updated_count = 0
        sessions = db.query(ImageSession).all()
        ...
        for sess in sessions:
            ...
            tasks = db.query(Task).filter(Task.session_id == sess.id, Task.task_type == "image").order_by(Task.id.asc()).all()
